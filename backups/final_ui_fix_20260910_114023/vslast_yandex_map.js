          ...
          function tryCreate() {
            ...
            if (!element) {
              ...
              if (attempts >= maxAttempts) {
                reject(new Error('Yandex map element not found after waiting: ' + elementId));
                return;
              }
            ...
            try {
              ...
              self.maps[elementId] = { map: map, marker: marker };

              map.events.add('boundschange', function (event) {
                if (!event.get('newZoom')) return;

                var center = map.getCenter();
                window.dispatchEvent(new CustomEvent('vslast-map-moved', {
                  detail: {
                    id: elementId,
                    latitude: center[0],
                    longitude: center[1],
              ...
              window.dispatchEvent(new CustomEvent('vslast-map-ready', {
                detail: { id: elementId }
              }));
