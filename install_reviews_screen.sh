...
set -e
echo "=============================================="
echo " Всласть — установка экрана Отзывы"
echo "=============================================="
...
mkdir -p lib/screens
TARGET="lib/screens/reviews_screen.dart"

if [ -f "$TARGET" ]; then
  BACKUP="${TARGET}.before_reviews_$(date +%Y%m%d_%H%M%S).bak"
  cp "$TARGET" "$BACKUP"
...
echo "flutter analyze lib/screens/reviews_screen.dart"
echo
echo "Если ошибок нет:"
echo "flutter run"
echo
echo "Подключение из карточки товара:"
echo "import 'package:vslast_premium/screens/reviews_screen.dart';"
echo
echo "Navigator.of(context).push(MaterialPageRoute("
echo "  builder: (_) => ReviewsScreen("
echo "    productName: product.name,"
echo "    productPrice: '\${product.price.toInt()} ₽',"
echo "    productImage: product.image,"
echo "    rating: 4.9,"
echo "    reviewCount: 64,"
echo "  ),"
echo "));"
echo
echo "Если поле картинки называется иначе, замените только product.image."
