...
class AdminProductEditResult {
  ...
  const AdminProductEditResult.saved(Product this.product, AdminProductMeta this.meta) : deleted = false;
  const AdminProductEditResult.deleted() : product = null, meta = null, deleted = true;
}
...
class _AdminProductEditScreenState extends State<AdminProductEditScreen>
  ...
  void initState() {
    ...
    _yieldCtrl = TextEditingController(text: m.yieldWeight.isNotEmpty ? m.yieldWeight : (p?.weightLabel ?? ''));
    _costCtrl = TextEditingController(text: m.costPrice > 0 ? m.costPrice.toString() : '');
  ...
  // Пул демо-изображений для "добавления фото" — реальный выбор из
  // галереи/камеры телефона требует пакета image_picker (плюс строки
  // разрешений в Info.plist/AndroidManifest), который не подключён в
  // проект. Здесь — рабочая заглушка с тем же UX (добавить/удалить/
  // несколько фото), чтобы её было легко заменить на настоящий пикер.
  static const _demoPhotoPool = [
    'assets/images/bread_country.jpg',
    'assets/images/bread_classic.jpg',
    'assets/images/bread_french.jpg',
      ...
      builder: (context) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Добавить фото', style: AppTextStyles.rowLabel.copyWith(fontWeight: FontWeight.w700, fontSize: 16)),
              const SizedBox(height: 4),
              Text(
                'Выбор из галереи и камера появятся после подключения соответствующего пакета. Пока — демо-набор фото.',
              ...
              Wrap(
                spacing: 10,
                runSpacing: 10,
                children: _demoPhotoPool.map((path) {
                  final alreadyAdded = _photos.contains(path);
                  return GestureDetector(
                    ...
                    child: Opacity(
                      ...
                      child: ClipRRect(
                        ...
                        child: Image.asset(path, width: 68, height: 68, fit: BoxFit.cover),
                      ),
                    ),
                  );
                }).toList(),
  ...
  void _addPriceEntry() {
    ...
    if (price == null || price <= 0) {
      FadeToast.show(context, 'Сначала укажите корректную цену', icon: Icons.error_outline);
      return;
    }
    setState(() {
      _priceHistory = [
    ...
    FadeToast.show(context, 'Цена зафиксирована в истории', icon: Icons.check_circle_outline);
  }

  bool get _isFormValid => _nameCtrl.text.trim().isNotEmpty && (int.tryParse(_priceCtrl.text.trim()) ?? 0) > 0;

  void _save() {
    final name = _nameCtrl.text.trim();
    final price = int.tryParse(_priceCtrl.text.trim()) ?? 0;
    if (name.isEmpty) {
      FadeToast.show(context, 'Укажите название товара', icon: Icons.error_outline);
      _tabController.index = 0;
      return;
    }
    if (price <= 0) {
      FadeToast.show(context, 'Укажите корректную цену', icon: Icons.error_outline);
      _tabController.index = 1;
      return;
    }

    final id = widget.product?.id ?? 'local-${DateTime.now().millisecondsSinceEpoch}';
    final product = Product(
      id: id,
      name: name,
      price: price,
      imageUrl: _photos.isNotEmpty ? _photos.first : (widget.product?.imageUrl ?? _demoPhotoPool.first),
      category: _category,
      ...
      weightLabel: _yieldCtrl.text.trim().isNotEmpty ? _yieldCtrl.text.trim() : (widget.product?.weightLabel ?? '1 шт'),
      description: _descriptionCtrl.text.trim().isEmpty ? null : _descriptionCtrl.text.trim(),
      caloriesPer100g: widget.product?.caloriesPer100g,
      proteinPer100g: widget.product?.proteinPer100g,
      fatPer100g: widget.product?.fatPer100g,
      carbsPer100g: widget.product?.carbsPer100g,
    ...
    Navigator.of(context).pop(AdminProductEditResult.saved(product, meta));
  }
  ...
  Future<void> _confirmDelete() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('«${widget.product?.name}» будет удалён из каталога без возможности восстановления.'),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('Отмена')),
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(true),
            child: const Text('Удалить', style: TextStyle(color: Color(0xFFB5544A))),
          ),
        ],
      ),
    );
              ...
              if (widget.isEditing)
                InkWell(
                  onTap: () {
                    ...
                    FadeToast.show(context, _active ? 'Товар снова активен' : 'Товар скрыт из каталога', icon: Icons.visibility_outlined);
                  },
                  child: Padding(
                    padding: const EdgeInsets.symmetric(vertical: 12),
                    child: Row(children: [
                      Icon(_active ? Icons.visibility_off_outlined : Icons.visibility_outlined, size: 19, color: AppColors.primaryBrown),
                      const SizedBox(width: 14),
                      Text(_active ? 'Скрыть товар' : 'Показать товар', style: AppTextStyles.rowLabel),
                    ]),
                  ),
                ),
              InkWell(
                onTap: () {
                  Navigator.of(context).pop();
                  FadeToast.show(context, 'Дублирование товара — скоро', icon: Icons.copy_outlined);
                },
                child: Padding(
                  padding: const EdgeInsets.symmetric(vertical: 12),
                  child: Row(children: [
                    const Icon(Icons.copy_outlined, size: 19, color: AppColors.primaryBrown),
                    const SizedBox(width: 14),
  ...
  Widget _tabBar() => TabBar(
    ...
    tabs: const [
      ...
      Tab(icon: Icon(Icons.storefront_outlined, size: 17), text: 'Производство'),
  ...
  Widget _mainTab() => ListView(
    ...
    children: [
      ...
      TextField(
        ...
        decoration: _inputDecoration(hint: 'Пара предложений о товаре для карточки в каталоге'),
      ),
    ...
    child: ListView(
      ...
      children: [
        for (int i = 0; i < _photos.length; i++) ...[
          _photoThumb(_photos[i], isCover: i == 0, onRemove: () => _removePhoto(i)),
  ...
  Widget _photoThumb(String path, {required bool isCover, required VoidCallback onRemove}) => Stack(
    children: [
      ClipRRect(
        borderRadius: BorderRadius.circular(16),
        child: Image.asset(path, width: 96, height: 96, fit: BoxFit.cover),
      ),
      if (isCover)
        Positioned(
          left: 6,
          ...
          child: Container(
            ...
            child: const Text('Главное', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w600)),
          ),
        ),
      Positioned(
        right: 4,
  ...
  Widget _addPhotoTile() => GestureDetector(
    ...
    child: Container(
      ...
      child: const Icon(Icons.add_a_photo_outlined, color: AppColors.primaryBrown),
    ),
  );
        ...
        children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _textField(_priceCtrl, hint: '450', keyboardType: TextInputType.number, suffix: '₽'),
            ]),
          ),
          const SizedBox(width: 12),
          Expanded(
        ...
        children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _textField(_stockCtrl, hint: '0', keyboardType: TextInputType.number, suffix: 'шт.'),
            ]),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _textField(_minStockCtrl, hint: '0', keyboardType: TextInputType.number, suffix: 'шт.'),
            ]),
          ),
        ],
      ),
      const SizedBox(height: 16),
      _switchRow('Доступен для заказа', _orderable, (v) => setState(() => _orderable = v)),
      const SizedBox(height: 10),
      _switchRow('Показывать в каталоге', _showInCatalog, (v) => setState(() => _showInCatalog = v)),
      ...
      _card(
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(child: Text('История цены', style: AppTextStyles.rowLabel.copyWith(fontWeight: FontWeight.w700))),
                OutlinedButton.icon(
            ...
            if (_priceHistory.isEmpty)
              Padding(
                ...
                child: Text('Пока нет истории изменения цены', style: AppTextStyles.rowLabelMuted),
              )
            else
              ..._priceHistory.take(4).map(
                (entry) => Padding(
                  padding: const EdgeInsets.symmetric(vertical: 6),
                  child: Row(
                    children: [
                      Expanded(flex: 3, child: Text(_formatDate(entry.date), style: AppTextStyles.rowLabelMuted)),
                      Expanded(flex: 2, child: Text('${entry.price} ₽', style: AppTextStyles.rowLabel.copyWith(fontWeight: FontWeight.w600))),
                      Expanded(flex: 2, child: Text(entry.author, style: AppTextStyles.rowLabelMuted, textAlign: TextAlign.end)),
                    ],
                  ),
                ),
              ),
  ...
  Widget _compositionTab() => ListView(
    ...
    children: [
      _label('Состав'),
      Text('Ингредиенты через запятую — так же, как показывается покупателю на карточке товара.', style: AppTextStyles.rowLabelMuted.copyWith(fontSize: 12)),
      const SizedBox(height: 8),
      TextField(
        controller: _compositionCtrl,
        maxLines: 6,
        decoration: _inputDecoration(hint: 'Мука пшеничная высшего сорта, вода, закваска, соль'),
      ),
    ],
  );
        ...
        children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _textField(_costCtrl, hint: '156', keyboardType: TextInputType.number, suffix: '₽'),
            ]),
          ),
        ],
      ),
    ...
    if (!widget.isEditing || m == null) {
      return ListView(
        ...
        children: [
          Column(
            children: [
              const Icon(Icons.bar_chart_outlined, size: 40, color: AppColors.textSecondary),
  ...
  Widget _statTile(String label, String value) => Column(
    ...
    children: [
      ...
      const SizedBox(height: 4),
      Text(value, style: AppTextStyles.rowLabel.copyWith(fontWeight: FontWeight.w700, fontSize: 20)),
    ],
  );
              ...
              children: [
                Expanded(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    icon: const Icon(Icons.delete_outline, size: 18, color: Color(0xFFB5544A)),
  ...
  Widget _label(String text, {bool required = false}) => Padding(
    ...
    child: RichText(
      text: TextSpan(
        ...
        children: [
          TextSpan(text: text),
          if (required) const TextSpan(text: ' *', style: TextStyle(color: Color(0xFFB5544A))),
        ],
      ),
    ),
  );

  InputDecoration _inputDecoration({String? hint, String? suffixText}) => InputDecoration(
    hintText: hint,
    ...
    focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: AppColors.primaryBrown)),
  );

  Widget _textField(TextEditingController controller, {String? hint, TextInputType? keyboardType, String? suffix}) => TextField(
    ...
    child: DropdownButtonHideUnderline(
      child: DropdownButton<ProductCategory>(
        ...
        items: ProductCategory.values
            .map((c) => DropdownMenuItem(value: c, child: Text(c.label, style: AppTextStyles.rowLabel)))
            .toList(),
        onChanged: (v) {
  ...
  Widget _unitDropdown() => Container(
    ...
    child: DropdownButtonHideUnderline(
      child: DropdownButton<String>(
        value: _unit,
        isExpanded: true,
        items: _units.map((u) => DropdownMenuItem(value: u, child: Text(u, style: AppTextStyles.rowLabel))).toList(),
        onChanged: (v) {
  ...
  Widget _segmentedToggle({
    ...
    child: Row(
      children: [
        Expanded(child: _segmentButton(leftLabel, value, () => onChanged(true))),
        Expanded(child: _segmentButton(rightLabel, !value, () => onChanged(false))),
      ],
    ),
  );

  Widget _segmentButton(String label, bool selected, VoidCallback onTap) => GestureDetector(
    onTap: onTap,
  ...
  Widget _switchRow(String label, bool value, ValueChanged<bool> onChanged) => Row(
    children: [
      ...
      Switch(value: value, onChanged: onChanged, activeTrackColor: AppColors.primaryBrown),
    ],
  );
