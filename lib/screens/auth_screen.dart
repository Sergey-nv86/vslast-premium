...
/// C-ID генерируется сервером автоматически.
...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _submitLogin() async {
    ...
    final digits = _clientIdController.text.replaceAll(RegExp(r'\D'), '');
    final clientId = 'C-$digits';
    final password = _passwordController.text;

    if (digits.length != 6) {
      _showMessage('Введите 6 цифр ID клиента.');
      return;
    }
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('Ваш постоянный ID клиента:', style: AppTextStyles.body),
              ...
              Text(
                'Сохраните этот ID. Он используется '
                'для входа в приложение. Пароль также '
                ...
                children: [
                  ...
                  Text(
                    _mode == AuthMode.login
                        ...
                        : 'Получите персональный ID клиента для входа во Всласть',
  ...
  Widget _buildLogin() {
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  'Введите только 6 цифр. Префикс C- '
        ...
        Center(
          child: Text(
            'ID и пароль сохраняйте после регистрации.',
            textAlign: TextAlign.center,
      ...
      children: [
        Container(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Icon(Icons.auto_awesome, color: AppColors.primaryBrown),
              const SizedBox(width: 12),
              Expanded(
                child: Text(
                  'ID клиента будет создан автоматически '
                  'после регистрации. Телефон и e-mail '
                  'вводить не нужно.',
...
class _ModeSwitch extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: _tab('Вход', AuthMode.login)),
          Expanded(child: _tab('Регистрация', AuthMode.register)),
        ],
      ),
    );
  }
