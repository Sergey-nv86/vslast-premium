...
class _CatalogScreenState extends State<CatalogScreen> {
  ...
  Map<ProductCategory, double> _categoryOffsets(double viewportWidth) {
    ...
    for (final category in _shownCategories) {
      ...
      final gridHeight = rows == 0 ? 0.0 : rows * (itemWidth + _cardTextBlockHeight) + (rows - 1) * _gridMainAxisSpacing;
      y += 42 + gridHeight + 18;
    }
    return result;
  }
  ...
  void _scrollToCategory(ProductCategory category) {
    ...
    _scrollController.animateTo(target.toDouble(), duration: const Duration(milliseconds: 320), curve: Curves.easeOutCubic).then((_) {
      if (mounted) _updateActiveCategory();
    });
  }
          ...
          children: [
            CustomScrollView(
              ...
              slivers: [
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      SizedBox(
                        ...
                        child: Padding(
                          ...
                          child: _SearchHeader(controller: _searchController, autofocus: widget.autofocusSearch),
                        ),
                      ),
                      SizedBox(
                ...
                if (_isLoading)
                  ...
                  SliverToBoxAdapter(child: _CatalogErrorState(onRetry: _loadProducts))
                  ...
                  for (final category in categories) ...[
                    SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: Text(category.label, style: AppTextStyles.productName.copyWith(fontSize: 20, height: 1.2, fontWeight: FontWeight.w600)),
                      ),
                    ),
                    SliverPadding(
                      padding: EdgeInsets.fromLTRB(_horizontalPadding, 0, _horizontalPadding, category == categories.last && cart.isEmpty ? 28 : 18),
                      sliver: Builder(builder: (context) {
                        final products = _productsFor(category);
                        return SliverGrid(
                          gridDelegate: _gridDelegate(context),
                          delegate: SliverChildBuilderDelegate(
                            (context, index) => ProductCard(product: products[index], onOpenDetails: _openProductDetails),
            ...
            if (!cart.isEmpty)
              Positioned(
                ...
                right: 16,
                bottom: 8,
                child: CartSummaryBar(itemsCount: cart.totalCount, totalSum: cart.totalSum, onTap: _openCart),
              ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: InkWell(
        ...
        child: const SizedBox(width: 44, height: 44, child: Icon(Icons.chevron_left, size: 24, color: AppColors.primaryBrown)),
      ),
    );
  }
}
...
class _CatalogPinnedHeaderDelegate extends SliverPersistentHeaderDelegate {
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    return Material(
      ...
      child: SizedBox(width: double.infinity, height: height, child: ClipRect(child: child)),
    );
  }
  @override
  bool shouldRebuild(covariant _CatalogPinnedHeaderDelegate oldDelegate) => oldDelegate.height != height || oldDelegate.child != child;
}
...
class _SearchHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        const Padding(padding: EdgeInsets.only(left: 16), child: Icon(Icons.search, size: 22, color: AppColors.textSecondary)),
        ...
        Expanded(
          child: TextField(
            ...
            style: AppTextStyles.searchHint.copyWith(color: AppColors.textPrimary, fontSize: 15),
        ...
        if (controller.text.isNotEmpty)
          IconButton(tooltip: 'Очистить', onPressed: controller.clear, icon: const Icon(Icons.close, size: 20, color: AppColors.textSecondary))
        else
          const Padding(padding: EdgeInsets.only(right: 10), child: Icon(Icons.qr_code_scanner_rounded, size: 21, color: AppColors.primaryBrown)),
      ]),
    );
  }
}
...
class _CategoryBar extends StatelessWidget {
  ...
  const _CategoryBar({required this.categories, required this.activeCategory, required this.onCategoryTap, required this.onAllTap});
  @override
  Widget build(BuildContext context) {
    return Container(
      color: AppColors.background,
      ...
      child: ListView(
        ...
        children: [
          ...
          ...categories.map((category) => Padding(
            ...
            child: CategoryChip(label: category.label, selected: activeCategory == category, onTap: () => onCategoryTap(category)),
          )),
        ],
      ),
    );
...
class _CatalogLoadingState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.fromLTRB(_CatalogScreenState._horizontalPadding, 8, _CatalogScreenState._horizontalPadding, 24),
